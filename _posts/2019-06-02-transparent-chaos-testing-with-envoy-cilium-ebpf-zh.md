...
categories: envoy cilium ebpf chaos
---
...
基于路径的路由（path based routing）或者 7层路由、金丝雀发布（canary release）、
自动重试、熔断、限流等等。
...
比。这是Envoy 的一个典型使用场景，也是我们混沌测试要用到的模式。
...
户态上下文切换）代价比较大。但注意Go 也并非每个包都会产生一次上下文切换，通常是
...
对于 `l7proto`，我们支持 Cassandra、Kafka、Memcached等等，你要选择其中一种，并且
