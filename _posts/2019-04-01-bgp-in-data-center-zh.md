...
本章介绍在给定应用需要和预期规模的前提下，如何为现代数据中心设计网络（network
...
。但是，它也可以用在数据中心内部。现在数据中心中使用最广泛的路由协议就是 BGP。
...
1. 规模（Scale）
    ...
    在过去，几百台服务器就已经是一个大数据中心；现在，现代数据中心一个机
    房可能就有上万台服务器。
...
数据中心内部是该使用内部网关协议（iBGP）还是外部网关协议（eBGP）？很多人觉得应该
是iBGP，因为在数据中心内部，但其实不是。
...
当一个节点挂到后，其他节点陆续回撤（withdraw）可达路由时，导致网络内大量的 BGP消
...
两条路由相同的判断标准：以上八个条件都相同。其中，AS_PATH 字段一定要是 ASN
...
**Clos 网络的任意两个 node 直接只有一条链路**，也即，**任何两个 node 之间都只有唯一的
一对接口。因此不会出现上面提到的问题**。如果是有多条链路，的确会无法区分从哪个接口
...
network）。即，**它提供了一种下一跳是 IPv6 地址的 IPv4 路由**（carray IPv4 routes
