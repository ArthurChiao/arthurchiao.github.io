...
2. 如果页面超过一个 MTU，会分为多个 packet 进行传输（后面会看到，确实超过 MTU了）
2. TCP 断开连接的**四次挥手**
...
1. `#1` length为 0，因为 SYN 包不带 TCP payload，所有信息都在 TCP header

#### 第二次握手: SYN+ACK

`#2` 的 ack 是 `3310420140`，等于 `#1` 的 seq 加 1，这就说明，`#2` 是 `#1` 的应
答包。
...
5. `#8`: server 向 client 端继续发送179字节数据
...
server的应答包。要跟进这个问题，就需要在 server 端一起抓包，看应答包是否有发出来
。本文不对此展开。
