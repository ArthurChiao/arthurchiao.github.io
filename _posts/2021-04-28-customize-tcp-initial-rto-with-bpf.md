---
layout    : post
title     : "Customize TCP initial RTO (connect timeout) with BPF"
date      : 2021-04-28
...
TCP connection timeout (retransmission timeout, RTO) has been initialized as a
**<mark>harcoded value of 1 second</mark>** in Linux kernel (not configurable).
A BPF hook has been added to the `connect` operation since `4.13`, which gives
users a chance to dynamically override the hardcode (instead of re-compiling
kernel) with BPF programs.

This post explores that facility, and implements **<mark>one such program with
several lines of BPF code</mark>**.  Glad if it helps to understand why
...
In Linux kernel (`<= 4.12`), the **<mark>initial timeout value</mark>** when establishing a new TCP connection
...
## 1.2 Confirm the initial RTO (kernel <= 4.12)

Confirm it by initiating a TCP request to a non-existing service (so it will always timeout):
