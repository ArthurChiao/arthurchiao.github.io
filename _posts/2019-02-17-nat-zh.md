...
再后面，会介绍几种其他的可能性，例如重定向，或者如何使用代理（proxy）绕过防火
...
这个（iptables）table 里。这个表有三个预置的 chain：`PREROUTING`, `OUTPUT` 和
`POSTROUTING`。

<p align="center"><img src="/assets/img/nat-zh/nat-chains.gif" width="80%" height="80%"></p>

`PREROUTING` 和 `POSTROUTING` 是最重要的 chain。如名字所示，`PREROUTING` chain 负
责处理刚刚到达网络接口的数据包，这时还没有做路由判断，因此还不知道这个包是发往本
机（local），还是网络内的其他主机。包经过 `PREROUTING` chain 之后，将进行路由判
...
用它了。NAT 最广泛的使用场景应该就是：**一个私有子网的所有机器共享一个公网连接**。
接下来我们就从这个例子开始。
...
我们希望实现的是：**从本地网络发出的、目的是公网的包，将发送方地址修改为路由器
的地址。**

接下来假设路由器的本地网络走 `eth0` 端口，到公网的网络走 `eth1` 端口。那么**如
下命令就能完成我们期望的功能**：
...
* **<mark><code>-j MASQUERADE</code></mark>** - 规则匹配成功后的动作是 **masquerade**
（伪装）数据包，例如将源地址修改为路由器地址
...
现在，本地计算机可以访问公网，但反过来，我们看看公网上的机器访问本地机器会是
什么情况。

公网上的机器要和本地机器建立连接的话，它唯一能利用的信息就是用路由器的 IP 地址
...
前面的将本地私有网络连接到公网的例子中，我们已经使用了 Source NAT（SNAT
...
显然，接收端修改**必须在做路由决策之前，因此 DNAT 适用于 `PREROUTING` 和
`OUTPUT` （本地生成的包）chain**。
...
例如透明代理的功能。和 DNAT 一样，REDIRECT 适用于 `PREROUTING` 和 `OUTPUT` chain
。
...
本文第一个例子（将本地网络连接到公网）的晦涩（cryptic）指令可能让你有些困惑。
...
假设我们有一个本地网络，通过 NAT 连接到公网。我们需要在路由器的 8080 端口运行
