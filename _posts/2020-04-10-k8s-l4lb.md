---
layout    : post
title     : "L4LB Solution for Kubernetes: Theory and Practice with Cilium+BGP+ECMP"
date      : 2020-04-10
...
you may face the need to **exposing your services inside a Kubernetes cluster to
...
Kubernets provides two **models** for this scenario:
...
cluster, intended for providing **corparation-wide** DNS service.

Now the question is: **how would your legacy applications** (e.g. running on
baremetal or VM) or **application in other Kubernetes clusters access this DNS
...
1. DNS provides service at **UDP** (or TCP) **port `53`** (thus **layer 4**).
...
1. assign an VIP to the DNS cluster, all clients access the service by VIP
1. forward traffic from VIP to specific backend with load balancing algorithms
...
* on the one hand, it must be reachable from outside world (while most
...
1. **Persistent/invarient L4 entrypoint for accesing Kubernetes service from outside
   world**: the entrypoint, e.g. VIP, should be unique and invarient, hiding
   users/clients from backend changes from.
2. **Load balancing**: load balance requests between different backends.
3. **Reactive on backend changes**: update forwarding rules according to backend
...
2. Running a **BGP agent** on each L4LB node, annoucing a specific CIDR to
   underlying network, e.g. `10.1.0.1/24`. IPs in this CIDR will be used as
...
Traffic path when accessing the example service from outside world:

<p align="center"><img src="/assets/img/k8s-l4lb/l4lb-traffic-path.png" width="85%" height="85%"></p>
<p align="center">Fig. 3.2 Traffic path when accesing example service from outside of Kubernetes cluster</p>
...
If not familir with BGP announcement, you could simply think of it as
...
This solves the `1st requirement`: **persistent/invarient (L4) entrypoint**.
...
to forward traffic from VIPs (which are holded by L4LB nodes) to backend pods.

This solves the `2nd and 3rd requirement`: **load balancing** and **timely
react on backend changes**.
...
But one part is is still missing: **there are no rules in the kernel to redirect
those packet into `cilium-agent`'s processing scope**. So without additional work,
...
scheme work for non direct routing cases, that's beyong the scope of this post).
...
Configure to survive host reboot:
...
latter is beyong the scope of this post. Refer to
...
For `bird`, add following configuration to `/etc/bird.conf`,
...
It's important to understand that, in this L4LB scheme, there is no
...
Ingress provides persistent L7 entrypoints for acessing services inside Kubernetes
